...
from model_only_classify_cold_start_drug import myModel
...
def getSimTopK(sim_info,knn=10):
...
def get_data(train_index, test_index, raw_freq):
    ...
    # with open("../data/664_data/NRFSE/drug_sim_fp_664_664_np.pkl",'rb') as f:
    #     drug_5fp_jaccard = pickle.load(f)
    # with open("../data/664_data/NRFSE/drug_sim_gofs_664_664_np.pkl",'rb') as f:
    #     drug_go_jaccard = pickle.load(f)
    ...
    with open("../data/664_data/NRFSE/side_sim_shhp_994_994_np.pkl",'rb') as f:
        side_semantic_sim = pickle.load(f)
    np.fill_diagonal(side_semantic_sim,0)
    raw_asso = np.copy(raw_freq)
    raw_asso[raw_asso!=0] = 1
    asso_masked = np.copy(raw_asso)
    asso_masked[test_index,:] = 0
    drug_asso_sim = cosine_similarity(asso_masked)
    side_asso_sim = cosine_similarity(asso_masked.T)
    ...
    # train_drug_sim_asso = drug_asso_sim[train_index,:][:,train_index]
    ...
    # test_drug_sim_asso = drug_asso_sim[test_index,:][:,train_index]
    ...
    test_drug_sim_info = [test_drug_sim_5fp, test_drug_sim_gofs, test_drug_sim_disease, test_drug_sim_gene]
    ...
    raw_asso = np.copy(raw_freq)
    raw_asso[raw_asso!=0] = 1
    train_asso = raw_asso[train_index,:]
    # test_asso = raw_asso[test_index,:]
    ...
    for i in range(len(train_drug_sim_info)):
        for j in range(len(train_side_sim_info)):
            interactor_matrix_asso, feature_matrix_asso, num_features_asso = getHeteNetData(train_drug_sim_info[i], train_side_sim_info[j],train_asso, factor=5)
            inm_list_asso.append(interactor_matrix_asso)
            feat_list_asso.append(feature_matrix_asso)

    drug_autocorrelation = np.matmul(train_asso,train_asso.T)
    drug_autocorrelation[drug_autocorrelation!=0] = 1
    side_autocorrelation = np.matmul(train_asso.T,train_asso)
    side_autocorrelation[side_autocorrelation!=0] = 1


    return [train_drug_sim_info,train_side_sim_info,inm_list_asso,feat_list_asso,drug_autocorrelation,side_autocorrelation,test_drug_sim_info]

def train_test(raw_frequency,frequency_masked,mask,new_index,fold, args):
    ...
    nonzero_count = np.count_nonzero(raw_frequency[train_index,:])
    zero_count = np.count_nonzero(raw_frequency[train_index,:] == 0)

    data_list = get_data(train_index,test_index,raw_frequency)
    raw_asso = np.copy(raw_frequency)
    raw_asso[raw_asso!=0] = 1



    # test_data_file_save_path = "../data/664_data/data_"+args.mode+"_seed_"+str(args.seed)+"/"+str(fold)+"_test_inm_list_classify.pkl"
    # if os.path.exists(test_data_file_save_path):
    #     with open(test_data_file_save_path,'rb') as f:
    #         test_input_data = pickle.load(f)
    # else:
    #     num_old = data_list[0][0].shape[0]
    #     test_inm_list = []
    #     test_feat_list = []
    #     for i in tqdm(range(len(test_index)), desc="get test inm..."):
    #         asso_info = raw_asso[train_index, :]
    #         asso_info = np.vstack((asso_info, np.zeros((1, asso_info.shape[1]))))
    #         inm_list = []
    #         feat_list = []
    #         for sim_i in range(len(data_list[0])):
    #             drug_sim_info = data_list[0][sim_i]
    #             this_sample_sim_info = data_list[4][sim_i][i, :].reshape((1, -1))
    #             drug_sim_info = np.hstack((drug_sim_info, this_sample_sim_info.T))
    #             this_sample_sim_info = np.hstack((this_sample_sim_info, np.array([1]).reshape((1, 1))))
    #             drug_sim_info = np.vstack((drug_sim_info, this_sample_sim_info))
    #             for sim_s in range(len(data_list[1])):
    #                 side_sim_info = data_list[1][sim_s]
    #                 inm, _, _ = getHeteNetData(drug_sim_info, side_sim_info, asso_info, factor=5)
    #                 inm_list.append(inm.to(args.device))
    #                 this_feat = np.hstack((data_list[4][sim_i][i, :], np.zeros(raw_asso.shape[1]))).reshape((1, -1))
    #                 feat = np.vstack((data_list[3][0][:num_old, :], this_feat, data_list[3][0][num_old:, :]))
    #                 feat = torch.Tensor(feat).to(args.device)
    #                 feat_list.append(feat)
    #         test_inm_list.append(inm_list)
    #         test_feat_list.append(feat_list)
    #     test_input_data = [test_inm_list, test_feat_list]
    #     with open(test_data_file_save_path,'wb') as f:
    #         pickle.dump(test_input_data,f)
    #
    # for i in range(len(test_input_data)):
    #     for j in range(len(test_input_data[i])):
    #         for k in range(len(test_input_data[i][j])):
    #             if type(test_input_data[i][j][k]) != torch.Tensor:
    #                 test_input_data[i][j][k] = torch.tensor(test_input_data[i][j][k],dtype=torch.float32).to(args.device)
    #             else:
    #                 test_input_data[i][j][k] = test_input_data[i][j][k].to(args.device)
    #             # del test_input_data[i][j][k]
    ...
    train_asso = raw_asso[train_index]
    new_side_index = np.where(np.all(train_asso == 0, axis=0))[0]
    train_asso = torch.Tensor(train_asso).to(args.device)
    test_asso = torch.Tensor(raw_asso[test_index]).to(args.device)
    train_index = torch.LongTensor(train_index).to(args.device)
    test_index = torch.LongTensor(test_index).to(args.device)


    new_side_index = torch.LongTensor(new_side_index).to(args.device)

    train_data = data_list[:-1]
    train_data.extend([train_index, train_asso])
    # test_data = [data_list[-1], data_list[1],test_input_data[0], test_input_data[1],test_index,test_asso]
    test_data = [data_list[-1], data_list[1], data_list[2], data_list[3],data_list[4],data_list[5],new_side_index, new_index, test_asso]
    ...
    model = myModel(args,drug_num=len(train_index),side_num=args.side_num,drug_feats = len(data_list[0]), side_feats = len(data_list[1]),pos_weight = zero_count/nonzero_count).to(args.device)
    ...
    folder = "./metrics_"+args.mode+"/classify/fold"+str(fold)+"/runs/train/loss" # 指定存储文件夹
    writer = SummaryWriter(log_dir=folder) # 实例化SummaryWriter类，每30秒写入一次到硬盘


    folder_test = "./metrics_"+args.mode+"/classify/fold"+str(fold)+"/runs/test/loss" # 指定存储文件夹
    writer_test = SummaryWriter(log_dir=folder_test) # 实例化SummaryWriter类，每30秒写入一次到硬盘

    folder_train_metric = "./metrics_"+args.mode+"/classify/fold"+str(fold)+"/runs/train/metric" # 指定存储文件夹
    writer_train_metric = SummaryWriter(log_dir=folder_train_metric) # 实例化SummaryWriter类，每30秒写入一次到硬盘

    folder_test_metric = "./metrics_"+args.mode+"/classify/fold"+str(fold)+"/runs/test/metric" # 指定存储文件夹
    ...
    for epoch in range(0, args.epochs):
        ...
        if (epoch)%25 == 0:
            ...
            t_i_auc, t_iPR_auc = verify(model, verify_loader, train_data, args)
            ...
            #if (epoch+1) == int(0.6*args.epochs):
            #    optimizer.param_groups[0]['lr']=0.0001
        ...
        # break
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/train_loss.txt", 'w') as train_loss_f:
        train_loss_f.write(str(train_loss))
    #
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/train_auc.txt", 'w') as train_auc_f:
        train_auc_f.write(str(train_auc))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/train_aupr.txt", 'w') as train_aupr_f:
        train_aupr_f.write(str(train_aupr))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/train_rmse.txt", 'w') as train_rmse_f:
        train_rmse_f.write(str(train_rmse))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/train_mae.txt", 'w') as train_mae_f:
        train_mae_f.write(str(train_mae))

    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/test_auc.txt", 'w') as test_auc_f:
        test_auc_f.write(str(test_auc))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/test_aupr.txt", 'w') as test_aupr_f:
        test_aupr_f.write(str(test_aupr))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/test_rmse.txt", 'w') as test_rmse_f:
        test_rmse_f.write(str(test_rmse))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/test_mae.txt", 'w') as test_mae_f:
        test_mae_f.write(str(test_mae))
    with open("./metrics_"+args.mode+"/classify/fold"+str(fold)+"/test_pcc.txt", 'w') as test_pcc_f:
    ...
    checkpointsFolder = "./metrics_"+args.mode+"/classify/model"
...
def train(model,train_loader,train_data,optimizer,args):
    ...
    for i, batch_drug in enumerate(tqdm(train_loader,desc="Train......",unit="batch"), 0):
        ...
        optimizer.zero_grad()
        pred_freq,loss = model(train_data[:6], batch_drug,train_data[-1][batch_drug].flatten())
        loss.backward()
        optimizer.step()
...
def verify(model, train_loader, train_data, args):
    ...
    with torch.no_grad():
        ...
        for i, batch_drug in enumerate(tqdm(train_loader,desc="Verify......",unit="batch"), 0):
            ...
            pred_asso,loss = model(train_data[:6], batch_drug,train_data[-1][batch_drug].flatten())
    ...
    with torch.no_grad():
        ...
        for i, batch_drug in enumerate(tqdm(test_loader,desc="Test......",unit="batch"), 0):
            if i not in new_index:
                continue
            batch_drug = batch_drug[0].to(args.device)
            # this_data = test_data[:2]
            # this_data.extend([test_data[2][batch_drug],test_data[3][batch_drug]])
            # pred_asso = model(this_data, old_drug_num,None, training=False)
            pred_asso = model(test_data[:7], batch_drug,None, training=False)
            temp = pred_asso.tolist()
            total_pred_asso_664.append(temp)
            this_pred_asso = pred_asso.squeeze().cpu()
            raw_asso = test_data[-1][batch_drug].squeeze().cpu()
            posi = this_pred_asso[torch.where(raw_asso!=0)]  # 预测结果中，与被掩盖的位置相对应的药副对为正样本
            nege = this_pred_asso[torch.where(raw_asso==0)]  # 找到mask[index].flatten()和train_label对应位置元素相减不为0，即不同的元素的索引
...
def test_save(model, test_loader, test_data, fold, args):
    ...
    with torch.no_grad():
        ...
        with open("./metrics_"+args.mode+"/classify/pred_value/"+str(fold)+"_pred_"+str(len(singleDrug_asso_auc))+"_994_np.pkl",'wb') as f:
...
def tenfold(args,seed=42):
    ...
    #保存每一折的最终预测结果
    try:
        File_Path = "./metrics_"+args.mode+"/pred_value/"
        print(File_Path)
        # 判断是否已经存在该目录
    ...
    for fold in range(0,10):
        ...
        try:
            File_Path = "./metrics_"+args.mode+"/fold"+str(fold)+"/during_train/"
            print(File_Path)
            # 判断是否已经存在该目录
            if not os.path.exists(File_Path):
                # 目录不存在，进行创建操作
        ...
        print(new_index)
        drugAUC, drugAUPR = train_test(raw_frequency, frequency_masked, mask,new_index, fold, args)
        ...
        # break
        try:
            File_Path = "./metrics_"+args.mode+"/classify/total"
            print(File_Path)
            # 判断是否已经存在该目录
            if not os.path.exists(File_Path):
                # 目录不存在，进行创建操作
            ...
            print("新建目录失败：" + msg)
        with open("./metrics_"+args.mode+"/classify/total/total_auc.txt", 'w') as total_auc_f:
            total_auc_f.write(str(total_auc))
        with open("./metrics_"+args.mode+"/classify/total/total_aupr.txt", 'w') as total_aupr_f:
            total_aupr_f.write(str(total_pr_auc))
        with open("./metrics_"+args.mode+"/classify/total/total_rmse.txt", 'w') as total_rmse_f:
            total_rmse_f.write(str(total_rmse))
        with open("./metrics_"+args.mode+"/classify/total/total_mae.txt", 'w') as total_mae_f:
            total_mae_f.write(str(total_mae))
        with open("./metrics_"+args.mode+"/classify/total/total_pcc.txt", 'w') as total_pcc_f:
