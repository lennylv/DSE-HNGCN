...
class hetenet(nn.Module):
    def __init__(self, obj_num,feat_num,out_dim):
        ...
        self.fc = nn.Linear(self.out_dim*3,self.out_dim)
    ...
    def forward(self,interactor_matrix,features,obj_index, training=True):
        ...
        # hidden = torch.cat((hidden11,hidden12,hidden13),dim=1)
        # info = self.fc(hidden)
...
class myModel(nn.Module):
    ...
    def forward(self, data_list, drug_index,true_label,training=True):
        if training:
            drug_sim_info, side_sim_info, inm_list_asso, feat_list_asso = data_list
            ...
            loss_classify = torch.sum((scores_asso[x1]-true_label[x1])**2)
