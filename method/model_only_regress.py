...
class myModel(nn.Module):
    ...
    def forward(self, data_list, drug_index,true_label,training=True):
        drug_sim_info,side_sim_info,inm_list,feat_list,inm_list_asso,feat_list_asso,inm_list_sim,feat_list_sim,origin_info_list = data_list
        ...
        if training:
            ...
            # x1 = torch.where(true_label == 1)
            # x2 = torch.where(true_label == 2)
            # x3 = torch.where(true_label == 3)
            # x4 = torch.where(true_label == 4)
            # x5 = torch.where(true_label == 5)
            #temp1 = x1[0].shape[0]
            #temp2 = x2[0].shape[0]
            #temp3 = x3[0].shape[0]
            #temp4 = x4[0].shape[0]
            ...
            #               + (temp1/temp2)*torch.sum((scores[x2] - true_label[x2]) ** 2)\
            #               + (temp1/temp3)*torch.sum((scores[x3] - true_label[x3]) ** 2)\
            #               + (temp1/temp4)*torch.sum((scores[x4] - true_label[x4]) ** 2)\
            #               + (temp1/temp5)*torch.sum((scores[x5] - true_label[x5]) ** 2) \
            #               + 0.05 * torch.sum((scores[x0] - 0) ** 2)
            loss_regress = torch.sum((scores[x1] - true_label[x1]) ** 2)
            # loss_classify = torch.sum((scores_asso[x1] - 1) ** 2) + 0.05 * torch.sum((scores_asso[x0] - 0) ** 2)
            # loss = loss_regress + loss_classify
            return scores, loss_regress
        else:
            return scores